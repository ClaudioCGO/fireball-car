...
void BMI160GenClass::i2c_init()
{
  ...
  // Don't call Wire.begin() again - it's already initialized in your setup()
  // Just verify the device is there with a proper read
  
  Wire.beginTransmission(i2c_addr);
  Wire.write(0x00);  // Write chip ID register address
  if( Wire.endTransmission() != 0 ) {
      Serial.println("BMI160GenClass::i2c_init(): I2C transmission failed.");
      return;
  }
  
  // Now read the chip ID back
